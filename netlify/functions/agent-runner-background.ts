...
export default async (req: Request) => {
  ...
  try {
    ...
    const model = AGENT_MODEL_MAP[agent_name] || 'llama-3.3-70b';
    const response = await callGroqWithRateLimit(model, workspace);
...
async function callGroqWithRateLimit(model: string, workspace: any) {
  // Rate limit check
