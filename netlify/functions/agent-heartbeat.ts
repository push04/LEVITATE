...
export default async () => {
  ...
  for (const job of dueJobs ?? []) {
    ...
    await fetch(`${process.env.NEXT_PUBLIC_PLATFORM_URL}/.netlify/functions/agent-runner-background`, { method: 'POST', body: JSON.stringify({ agent_name: job.agent_name, workspace_id: job.workspace_id }) });
