...
import { X, Loader2, Briefcase, Calendar, DollarSign, Save, Trash2 } from 'lucide-react';
...
interface Project {
    ...
    description: string;
    due_date: string;
    budget_estimate?: string; // Might be extracted or stored separately
    budget?: number; // DB column
}
    ...
    useEffect(() => {
        if (project) {
            setFormData({
                ...
                budget_estimate: project.budget ? project.budget.toString() : '' // Use budget column if available
            });
        }
    }, [project]);
        ...
        try {
            const { error } = await supabase
                ...
                .update({
                    ...
                    budget: formData.budget_estimate ? parseFloat(formData.budget_estimate) : null
                })
                .eq('id', project.id);
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                ...
                                <div className="pt-4 flex gap-3">
                                    <button
                                        ...
                                        className="px-4 py-2.5 rounded-xl font-medium bg-red-500/10 text-red-500 hover:bg-red-500/20 transition-colors flex items-center justify-center gap-2"
                                    ...
                                    <button
                                        type="button"
                                        onClick={onClose}
                                        className="px-6 py-2.5 rounded-xl font-medium bg-[var(--secondary)] hover:bg-[var(--border)] transition-colors"
                                    >
                                        Cancel
                                    </button>
                                    <button
                                        type="submit"
                                        disabled={loading}
                                        className="px-8 py-2.5 rounded-xl font-medium bg-indigo-600 text-white hover:bg-indigo-700 transition-colors flex items-center justify-center gap-2 disabled:opacity-50"
                                    >
