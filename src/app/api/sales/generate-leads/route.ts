...
export const runtime = 'nodejs'; // Node.js runtime for Cheerio
    ...
    const stream = new ReadableStream({
        async start(controller) {
            ...
            try {
                ...
                // Shuffle modifiers and take top 12 to generate diversity
                const selectedModifiers = modifiers.sort(() => 0.5 - Math.random()).slice(0, 12);
                ...
                // DATA CAP INCREASED TO 100
                potentialLeads = potentialLeads.slice(0, 100);
                ...
                if (deepScan) {
                    ...
                    const concurrency = 6; // High concurrency
                    ...
                    for (let i = 0; i < potentialLeads.length; i += concurrency) {
                        ...
                        await Promise.all(batch.map(async (r) => {
                            ...
                            try {
                                ...
                                const timeoutId = setTimeout(() => controller.abort(), 10000); // Faster timeout
                                ...
                                if (response.ok) {
                                    ...
                                    if (lead.email) {
                                        lead.score += 40;
                                        sendUpdate('log', `  ✔ ${lead.title.substring(0, 15)}... (Email Found!)`);
                                    } else {
