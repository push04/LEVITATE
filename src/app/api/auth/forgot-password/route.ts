import { NextResponse } from 'next/server';
import { createClient } from '@supabase/supabase-js';
import nodemailer from 'nodemailer';
import crypto from 'crypto';
import { getEmailTemplate } from '@/lib/email-template';
import { getSupabaseServiceRoleKey, getSupabaseUrl } from '@/lib/supabase-env';

function getSupabaseAdmin() {
    return createClient(getSupabaseUrl(), getSupabaseServiceRoleKey());
}

const GENERIC_SUCCESS = { success: true, message: 'If an account exists, a reset link has been sent.' };
const EMAIL_REGEX = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

async function findUserByEmail(email: string) {
    const supabaseAdmin = getSupabaseAdmin();
    let page = 1;
    while (page <= 20) {
        const { data: usersData, error } = await supabaseAdmin.auth.admin.listUsers({ page, perPage: 100 });
        if (error) throw error;

        const found = usersData?.users.find((user) => user.email?.toLowerCase() === email);
        if (found) return found;

        if (!usersData || usersData.users.length < 100) break;
        page++;
    }
    return null;
}

export async function POST(request: Request) {
    try {
        const supabaseAdmin = getSupabaseAdmin();
        const body = await request.json();
        const email = String(body?.email || '').trim().toLowerCase();

        if (!email || email.length > 254 || !EMAIL_REGEX.test(email)) {
            // Keep response generic to reduce account enumeration.
            return NextResponse.json(GENERIC_SUCCESS);
        }

        const user = await findUserByEmail(email);

        if (!user) {
            return NextResponse.json(GENERIC_SUCCESS);
        }

        // Store a hash of the reset token so database leaks do not expose valid reset links.
        const token = crypto.randomBytes(48).toString('hex');
        const tokenHash = crypto.createHash('sha256').update(token).digest('hex');
        const expiresAt = new Date(Date.now() + 1000 * 60 * 60); // 1 hour from now

        // Best-effort cleanup for stale rows.
        await supabaseAdmin
            .from('password_resets')
            .delete()
            .eq('email', email)
            .eq('used', false)
            .lt('expires_at', new Date().toISOString());

        const { error: dbError } = await supabaseAdmin
            .from('password_resets')
            .insert({
                email,
                token: tokenHash,
                expires_at: expiresAt.toISOString(),
                used: false
            });

        if (dbError) {
            console.error('DB Error:', dbError);
            return NextResponse.json({ error: 'Failed to process request' }, { status: 500 });
        }

        // 4. Send Email
        const smtpPort = parseInt(process.env.SMTP_PORT || '587', 10);
        const smtpSecure = process.env.SMTP_SECURE === 'true' || smtpPort === 465;
        // F-008: insecure TLS only permitted in non-production dev environments
        const allowInsecureTls = process.env.NODE_ENV !== 'production' && process.env.SMTP_ALLOW_INSECURE_TLS === 'true';
        const transporter = nodemailer.createTransport({
            host: process.env.SMTP_HOST,
            port: smtpPort,
            secure: smtpSecure,
            auth: {
                user: process.env.SMTP_USER,
                pass: process.env.SMTP_PASS,
            },
            ...(allowInsecureTls ? { tls: { rejectUnauthorized: false } } : {})
        });

        const resetLink = `${process.env.NEXT_PUBLIC_APP_URL}/reset-password?token=${token}`;

        await transporter.sendMail({
            from: process.env.SMTP_FROM || '"Levitate Support" <support@levitatelabs.com>',
            to: email,
            subject: 'Reset Your Password - Levitate Labs',
            html: getEmailTemplate({
                title: 'Password Reset Request',
                recipientName: 'User',
                message: `You requested a password reset for your Levitate Labs account.
                
                Click the button below to reset your password.
                
                <em>This link is valid for 1 hour.</em>`,
                ctaText: 'Reset Password',
                ctaLink: resetLink,
                footerText: 'If you didn\'t request this, you can ignore this email.\nTeam Levitate Labs',
                warning: true
            })
        });

        return NextResponse.json({ success: true, message: 'Reset link sent' });

    } catch (error: unknown) {
        console.error('Forgot Password Error:', error);
        // Keep the external behavior generic on failure too.
        return NextResponse.json(GENERIC_SUCCESS);
    }
}
