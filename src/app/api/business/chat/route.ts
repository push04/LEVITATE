...
export async function POST(req: NextRequest) {
  ...
  const [
    ...
    supabase.from('companies').select('name,industry,city,website').eq('id', company_id).maybeSingle(),
    supabase.from('leads').select('id', { count: 'exact', head: true }).eq('company_id', company_id),
    supabase.from('leads').select('business_name,name,city,service_category,status,budget,email,phone').eq('company_id', company_id).order('created_at', { ascending: false }).limit(15),
    ...
    supabase.from('leads').select('status').eq('company_id', company_id),
    supabase.from('leads').select('city').eq('company_id', company_id).not('city', 'is', null),
  ...
  const recentLeadsList = (recentLeadsRes.data ?? []).slice(0, 10).map(l =>
    `• ${l.business_name || l.name || '?'} — ${l.city || ''} ${l.service_category || ''} [${l.status || 'New'}]${l.budget ? ` ₹${l.budget}` : ''}`
  ).join('\n')
...
━━ BUSINESS CONTEXT ━━
Company: ${co?.name ?? '?'} | Industry: ${co?.industry ?? 'unknown'} | City: ${co?.city ?? 'unknown'}
...
Keep replies under 200 words. Use bullet points for lists. Be direct and actionable.`
