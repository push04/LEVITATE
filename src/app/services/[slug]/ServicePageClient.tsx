    ...
    return (
        <div className="min-h-screen pt-16">
            ...
            <section className={`relative py-20 bg-gradient-to-br ${category.color} overflow-hidden`}>
                <div className="absolute inset-0 opacity-10" style={GRID_BG_STYLE} />
                            ...
                            >
                                ...
                                <div className="flex gap-4 mt-8">
                                    <motion.a
                                        ...
                                        whileTap={{ scale: 0.95 }}
                                        className="px-8 py-4 bg-white text-gray-900 rounded-xl font-semibold hover:shadow-lg transition-shadow"
                                    >
                                        Get Started
                                    </motion.a>
                                    <motion.a
                        ...
                        >
                            ...
                            {service.longDescription.split('\n\n').map((block, index) => {
                                ...
                                if (block.trim().startsWith('###')) {
                                    return (
                                        <h3 key={index} className="text-xl font-bold text-[var(--foreground)] mt-6 mb-3">
                                            {block.replace(/^###\s+/, '')}
                                        </h3>
                                    );
                                }
                                ...
                                if (block.trim().startsWith('- ')) {
                                    ...
                                    return (
                                        <ul key={index} className="list-disc pl-5 space-y-2 mb-4 text-[var(--muted)]">
                                            {items.map((item, i) => (
                                                <li key={i} dangerouslySetInnerHTML={{
                                                    __html: DOMPurify.sanitize(item.replace(/^- /, '').replace(/\*\*(.*?)\*\*/g, '<strong class="text-[var(--foreground)]">$1</strong>'))
                                                }} />
                                            ))}
                                        </ul>
                                    );
                                ...
                                return (
                                    <p key={index} className="mb-4 text-lg text-[var(--muted)] leading-relaxed"
                                        dangerouslySetInnerHTML={{
                                            __html: DOMPurify.sanitize(block.replace(/\*\*(.*?)\*\*/g, '<strong class="text-[var(--foreground)]">$1</strong>'))
                                        }}
                                    />
                                );
                            })}
                        ...
                        {service.processSteps.map((step, i) => (
                            ...
                            >
                                ...
                                {i < service.processSteps.length - 1 && (
                                    <ArrowRight className="hidden md:block absolute -right-4 top-1/2 -translate-y-1/2 w-6 h-6 text-[var(--muted)] z-10" />
                                )}
                            </motion.div>
                        ))}
                    </div>
                        ...
                        {service.faq.map((item, i) => (
                            ...
                            >
                                <button
                                    ...
                                    className="w-full p-6 text-left flex items-center justify-between hover:bg-[var(--secondary)]/50 transition-colors"
                                >
