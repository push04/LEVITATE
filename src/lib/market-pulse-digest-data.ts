...
// Shared by the paid business dashboard route and the invite-only Market
// Pulse trial route - both show the exact same real digest, predictions,
// news, and track record; only the access-gating differs between them.
export async function getMarketPulseDigestPayload(requestedDate?: string | null) {
  ...
  if (availableDates.length === 0) {
    return { digestDate: null, digest: [], news: [], availableDates: [], trackRecord: await loadTrackRecord(supabase) }
  }
